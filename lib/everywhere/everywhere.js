...
a
.use(BASE, express.static(path.normalize(__dirname + '../../../public')))
.use(express.bodyParser())
.get(BASE + '/', function(q, r) {
    r.redirect(BASE + '/s/' + (+new Date()));
});

a.get(BASE + '/s/:room', function(q, r) {
    r.render('index.ejs', { base: BASE });
});
...
socket.on('connection', function(client) {
    ...
    client.on('message', function(message) {
        switch(message.type) {
            case 'meta':
                ...
                rooms[message.room].push(client.sessionId);
                break;
            case 'center':
                ...
                if (emperor === undefined || emperor === client.sessionId) {
                    ...
                    for (var c = 0; c < rooms[message.room].length; c++) {
                        if (rooms[message.room][c] !== client.sessionId) {
                            if (socket.clients[rooms[message.room][c]]) {
                                socket.clients[rooms[message.room][c]].send(message);
                            }
                        }
                    }
                    // }
    ...
    client.on('disconnect', function() {
        ...
        for (var room in rooms) {
            rooms[room] = _.without(rooms[room], client.sessionId);
